...
- **Steps**: Use Health Connect aggregate step totals directly; Health Connect priority settings handle overlap deduplication.
...
- **`StepHistoryUseCase`**: Queries Health Connect aggregate step totals for session and history windows.
...
- **Health Connect**: Write completed sessions, steps, and distance. Read aggregate steps for history and read weight history for the Stats trend chart. Do not write HR records because the WalkingPad owns calories and watches should own HR data.
...
- **`History & Stats`**: Charts and lists showing aggregate Health Connect steps.
...
- `StepHistoryUseCaseTest`: Mock Health Connect aggregate step totals and verify session reads, history grouping, and the default history window.
