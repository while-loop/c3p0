...
- **WalkingPad BLE Control Loop**: The WalkingPad does not rely on unsolicited status updates. After service discovery, the manager enables notifications, waits briefly for the protocol to settle, polls current status about every 750 ms, and serializes outbound commands with roughly 700 ms of spacing so preference sync, start/stop, mode, and speed commands are not sent back-to-back. The pad is only reported as connected once the WalkingPad protocol service is ready; speed and mode controls stay disabled until then, and readiness timeout errors include Android's discovered BLE services.
