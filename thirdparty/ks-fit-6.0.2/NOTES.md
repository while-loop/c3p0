...
- KS Fit exposes no-load stop UI and the likely encrypted KS property keys. On encrypted KS text transports, C3P0 sends them over the same text-property path as speed/mode: `props AuToStop 0/1` and `props NoloadStop <seconds>`. On FTMS+supplement pads, a captured supplement notification `72 81 02 02 00 f7` confirms the `autoStop` property id (`0x02`) with value `0` for off. C3P0 now subscribes to the supplement notify characteristic and saves the Profile setting only when a new `autoStop` response matches the requested value.
