...
- KS Fit exposes no-load stop UI and the likely encrypted KS property keys. On encrypted KS text transports, C3P0 sends them over the same text-property path as speed/mode: `props AuToStop 0/1` and `props NoloadStop <seconds>`. On FTMS+supplement pads, captured supplement notifications such as `72 81 02 02 00 f7` confirm the `autoStop` property id (`0x02`) is a boolean value, not the timeout. C3P0 now sends `autoStop=0/1`, subscribes to the supplement notify characteristic, and saves the Profile setting only when a new `autoStop` response matches the requested on/off value. The separate FTMS timeout setter is still unknown.
